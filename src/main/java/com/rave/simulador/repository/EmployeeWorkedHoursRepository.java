...
public interface EmployeeWorkedHoursRepository extends JpaRepository<EmployeeWorkedHours, Long> {
    ...
    @Query(value = "SELECT id, employee_id, worked_hours, worked_date from employee_worked_hours where id = :pid_job", nativeQuery = true)
    EmployeeWorkedHours findJobById(@Param("pid_job") Integer pid_job);
